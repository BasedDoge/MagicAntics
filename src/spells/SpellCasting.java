...
public class SpellCasting implements Listener {
    ...
    HashMap<String, HashMap<String, Long>> playerCooldown = new HashMap<>(); //String = player, hashmap = <spell name, time in milliseconds since last cast>
    HashMap<String, Long> spellCooldown = new HashMap<>(); // as above - Inner map
    ...
    public void onCast(PlayerInteractEvent e) {
        if (MAIR.SpellTomeCheck(e.getPlayer().getInventory().getItemInMainHand())) {
            if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
                ...
                switch (activeSpell) { //switch statement likely isn't the best way to store all spells maybe move them to another class
